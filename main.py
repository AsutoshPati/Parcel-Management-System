...
DB_FILE = "assets/database/PMS.db"
SQL_FILE = "assets/database/PMS.sql"
...
class Home(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        uic.loadUi("assets/pages/home_widget.ui", self)


class Signup(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/signup_widget.ui", self)
...
class ForgotPassword(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/forgot_password_widget.ui", self)


class Profile(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/profile_widget.ui", self)


class NewParcel(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/order_parcel_widget.ui", self)


class MyParcel(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/my_parcels_widget.ui", self)


class TrackParcel(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/track_parcel_widget.ui", self)


class Pickup(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/pickup_widget.ui", self)


class Deliver(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/deliver_widget.ui", self)


class ManageRole(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/manage_role_widget.ui", self)


class NewCity(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/expand_city_widget.ui", self)


class NewDepot(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/new_depot_widget.ui", self)


class AssignManager(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/assign_manager_widget.ui", self)


class Receive(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/receive_widget.ui", self)


class Dispatch(QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/dispatch_widget.ui", self)


class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi("assets/pages/main_win.ui", self)
...
def openMyParcelPage():
    ...
    conn = sql.connect(DB_FILE)
    query = (
        f"SELECT id FROM Parcel WHERE deliverTo = {current_id} "
        "ORDER BY id DESC"
    )
    ret = conn.execute(query)
    data = ret.fetchall()
...
def openReceivePage():
    ...
    if current_role == "Manager":
        ...
        show_message_box(
            msg_type="error", msg="Only managers can open this page"
        )
        return False
...
def doSignup():
    ...
    if len(name) < 3:
        show_message_box(
            msg_type="error", msg="Name should be atleast 3 characters long"
        )
        return
    for char in name:
    ...
    if not phno.isdigit() or len(phno) != 10:
        ...
        show_message_box(
            msg_type="error", msg="Phone number should be 10 digits long only"
        )
        return
    ...
    if len(address) < 10:
        show_message_box(
            msg_type="error", msg="Address should be atleast 10 characters long"
        )
        return
    ...
    try:
        ...
        show_message_box(
            msg_type="error", msg="Phone number already registered"
        )
    except Exception as err:
        ...
        query = (
            "INSERT INTO Authentication VALUES((SELECT "
            f"last_insert_rowid()), '{pwd}', 3)"
        )
        try:
            conn.execute(query)
        except sql.IntegrityError:
...
def doLogin():
    ...
    if not userid or not pwd:
        show_message_box(
            msg_type="error", msg="Please provide your credentials"
        )
        return
...
def changePassword():
    ...
    if phno and new_pwd and confirm_pwd:
        ...
        if new_pwd == confirm_pwd:
            if isValidPassword(new_pwd):
                ...
                if data:
                    user_id = data[0][0]
                    query = (
                        f"UPDATE Authentication SET password = '{new_pwd}' "
                    ...
                    show_message_box(
                        msg_type="info", msg="Your password has been updated"
                    )
                else:
                    show_message_box(
                        msg_type="error",
        ...
        show_message_box(
            msg_type="error", msg="Please provide the proper informations"
        )
...
def modifyProfile():
    ...
    if data[4] != desg:
        query.append(
            (", " if len(query) > 1 else "")
            + f"desgId=(SELECT id FROM Designation WHERE desgType='{desg}')"
        )
    ...
    try:
        if len(query) > 2 or data[5] != pwd:
            ...
            if data[5] != pwd:
                pwd_query = (
                    f"UPDATE Authentication SET password='{pwd}' "
                    f"WHERE userId={current_id}"
                )
        ...
        if "UserDetail.phno" in str(err):
            ...
            show_message_box(
                msg_type="error", msg=f"Unable to modify because \n{err}"
            )
    except Exception as err:
        show_message_box(
            msg_type="error", msg=f"Unable to modify because \n{err}"
...
def placeOrder():
    ...
    if not pay and not category:
        show_message_box(
            msg_type="error", msg="Please provide all required informations"
        )
        return
    ...
    try:
        ...
        show_message_box(
            msg_type="error", msg=f"Unable to place order because \n{err}"
        )
    else:
        query = """SELECT last_insert_rowid()"""
        ret = conn.execute(query)
        ...
        show_message_box(
            msg_type="info", msg=f"Order placed \nYour order id is {orderid}"
        )
    conn.close()
...
def checkParcelDetails():
    ...
    if not parcelid.isdigit():
        show_message_box(
            msg_type="error", msg="Please select a valid parcel ID"
        )
        return
    conn = sql.connect(DB_FILE)
    query = (
...
def completePickup():
    ...
    if not all(
        list(
            map(lambda x: bool(x), [parcelid, weight, length, breadth, ret_loc])
        )
    ):
        show_message_box(
            msg_type="error",
    ...
    try:
        ...
        show_message_box(
            msg_type="error", msg=f"Unable to pickup order because \n{err}"
        )
        return
    else:
        query = (
        ...
        try:
            ...
            try:
                ...
                conn.commit()
                show_message_box(
                    msg_type="info", msg="The order has been picked up"
                )
    conn.close()
    closePickupPage()
...
def checkBeforeDeliver():
    ...
    if len(data) != 0:
        ...
        show_message_box(
            msg_type="error", msg="The parcel has not yet picked up"
        )
        conn.close()
        return
...
def completeDeliver():
    ...
    try:
        ...
        show_message_box(
            msg_type="error", msg=f"Unable to deliver order because \n{err}"
        )
        return
    else:
        query = (
...
def completeReturn():
    ...
    try:
        ...
        show_message_box(
            msg_type="error", msg=f"Unable to return order because \n{err}"
        )
        return
    else:
        conn.commit()
...
def assignRole():
    ...
    try:
        ...
        conn.commit()
        show_message_box(
            msg_type="info", msg="New role has been assigned to the user"
        )
    except Exception as err:
        show_message_box(
            msg_type="error", msg=f"Unable to assign role because \n{err}"
        )
    else:
        closeManageRolePage()
    conn.close()
...
def expandCity():
    ...
    if len(city_name) < 3:
        ...
        try:
            ...
            openNewCityPage()
            show_message_box(
                msg_type="info", msg="You have expanded to a new city"
            )
        conn.close()
...
def searchCity():
    ...
    if not city_name:
        ...
        show_message_box(
            msg_type="info", msg=f"City id for {city_name} is {cityid}"
        )
        conn.close()
...
def findCity():
    ...
    if len(cityid) != 3:
        ...
        if len(data) == 0:
            show_message_box(
                msg_type="error", msg="No city found for given cityid"
            )
            return
        else:
...
def findManager():
    ...
    if len(managerid) != 6:
        ...
        if len(data) == 0:
            show_message_box(
                msg_type="error", msg="No manager found for given manager ID"
            )
            return
        else:
...
def inaugurateDepot():
    ...
    conn = sql.connect(DB_FILE)
    query = (
        f"INSERT INTO Depot(cityId, manager, address) VALUES({cityid}, "
        f"{managerid}, '{address}')"
    )
    try:
        conn.execute(query)
        ...
        show_message_box(
            msg_type="error", msg=f"Unable to inaugurate because \n{err}"
        )
    else:
        conn.commit()
...
def getDepotDetails():
    ...
    if len(depotid) != 3 and not depotid.isdigit():
        show_message_box(
            msg_type="error", msg="Please provide a valid depot ID"
        )
        return
...
def assignManager():
    ...
    if len(managerid) != 6:
        ...
        if len(data) == 0:
            ...
            query = (
                f"UPDATE Depot SET manager = {managerid} "
                f"WHERE id = {depotid}"
            )
            try:
                conn.execute(query)
                ...
                conn.commit()
                show_message_box(
                    msg_type="info", msg="New manager has been assigned"
                )
        conn.close()
...
def recieveParcel():
    ...
    if not depot_address:
        show_message_box(
            msg_type="error", msg="Please check the depot address first"
        )
        return
    ...
    if data:
        show_message_box(
            msg_type="error", msg="This parcel is already delivered"
        )
        conn.close()
        return
    ...
    if data:
        ...
        if str(data[1]) == depot_id:
            show_message_box(
                msg_type="error", msg="This parcel is already received"
            )
            conn.close()
            return
    ...
    try:
        ...
        show_message_box(
            msg_type="error", msg=f"Unable to recieve parcel due to \n{err}"
        )
    else:
...
def dispatchParcel():
    ...
    if not data:
        ...
        if data[0] == "Dispatched":
            show_message_box(
                msg_type="error", msg="The parcel is already dispatched"
            )
            conn.close()
            return
...
def showTrackingDetails():
    ...
    if not data:
        ...
        data = data[0]
        timestamp = datetime.strptime(data[1], "%d-%m-%Y %H:%M:%S").strftime(
            "%d-%b-%Y"
        )
...
if __name__ == "__main__":
    ...
    # create listeners
    ### home page buttons
    main_win.home.pushButton.clicked.connect(
        partial(travel, "forgot_password", "home")
    )
    main_win.home.pushButton_2.clicked.connect(doLogin)
    main_win.home.pushButton_3.clicked.connect(
        partial(travel, "signup", "home")
    )

    ### sigup page buttons
    main_win.signup.pushButton.clicked.connect(
        partial(travel, "home", "signup")
    )
    main_win.signup.pushButton_2.clicked.connect(doSignup)

    ### forgot_password page buttons
    main_win.forgot_password.pushButton.clicked.connect(
        partial(travel, "home", "forgot_password")
    )
    ...
    main_win.profile.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "profile")
    )
    main_win.profile.pushButton_3.clicked.connect(
        partial(travel, "pickup", "profile")
    )
    main_win.profile.pushButton_4.clicked.connect(
        partial(travel, "receive", "profile")
    )
    main_win.profile.pushButton_5.clicked.connect(
        partial(travel, "manage_role", "profile")
    )
    ...
    main_win.new_parcel.pushButton.clicked.connect(
        partial(travel, "profile", "new_parcel")
    )
    main_win.new_parcel.pushButton_3.clicked.connect(
        partial(travel, "pickup", "new_parcel")
    )
    main_win.new_parcel.pushButton_4.clicked.connect(
        partial(travel, "receive", "new_parcel")
    )
    main_win.new_parcel.pushButton_5.clicked.connect(
        partial(travel, "manage_role", "new_parcel")
    )
    main_win.new_parcel.pushButton_6.clicked.connect(placeOrder)
    main_win.new_parcel.pushButton_7.clicked.connect(
        partial(doLogout, "new_parcel")
    )
    main_win.new_parcel.pushButton_9.clicked.connect(
        partial(travel, "my_parcel", "new_parcel")
    )
    main_win.new_parcel.pushButton_10.clicked.connect(
        partial(travel, "track_parcel", "new_parcel")
    )

    ### my_parcel page buttons
    main_win.my_parcel.pushButton.clicked.connect(
        partial(travel, "profile", "my_parcel")
    )
    main_win.my_parcel.pushButton_3.clicked.connect(
        partial(travel, "pickup", "my_parcel")
    )
    main_win.my_parcel.pushButton_4.clicked.connect(
        partial(travel, "receive", "my_parcel")
    )
    main_win.my_parcel.pushButton_5.clicked.connect(
        partial(travel, "manage_role", "my_parcel")
    )
    main_win.my_parcel.pushButton_6.clicked.connect(checkParcelDetails)
    main_win.my_parcel.pushButton_7.clicked.connect(
        partial(doLogout, "my_parcel")
    )
    main_win.my_parcel.pushButton_8.clicked.connect(
        partial(travel, "new_parcel", "my_parcel")
    )
    main_win.my_parcel.pushButton_10.clicked.connect(
        partial(travel, "track_parcel", "my_parcel")
    )

    ### pickup page buttons
    main_win.pickup.pushButton.clicked.connect(
        partial(travel, "profile", "pickup")
    )
    main_win.pickup.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "pickup")
    )
    main_win.pickup.pushButton_4.clicked.connect(
        partial(travel, "receive", "pickup")
    )
    main_win.pickup.pushButton_5.clicked.connect(
        partial(travel, "manage_role", "pickup")
    )
    ...
    main_win.pickup.pushButton_9.clicked.connect(
        partial(travel, "deliver", "pickup")
    )

    ### deliver page buttons
    main_win.deliver.pushButton.clicked.connect(
        partial(travel, "profile", "deliver")
    )
    main_win.deliver.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "deliver")
    )
    main_win.deliver.pushButton_4.clicked.connect(
        partial(travel, "receive", "deliver")
    )
    main_win.deliver.pushButton_5.clicked.connect(
        partial(travel, "manage_role", "deliver")
    )
    ...
    main_win.deliver.pushButton_8.clicked.connect(
        partial(travel, "pickup", "deliver")
    )
    ...
    main_win.manage_role.pushButton.clicked.connect(
        partial(travel, "profile", "manage_role")
    )
    main_win.manage_role.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "manage_role")
    )
    main_win.manage_role.pushButton_3.clicked.connect(
        partial(travel, "pickup", "manage_role")
    )
    main_win.manage_role.pushButton_4.clicked.connect(
        partial(travel, "receive", "manage_role")
    )
    main_win.manage_role.pushButton_6.clicked.connect(findUserForMangeRole)
    main_win.manage_role.pushButton_7.clicked.connect(
        partial(doLogout, "manage_role")
    )
    main_win.manage_role.pushButton_9.clicked.connect(
        partial(travel, "assign_manager", "manage_role")
    )
    main_win.manage_role.pushButton_10.clicked.connect(assignRole)
    main_win.manage_role.pushButton_11.clicked.connect(
        partial(travel, "new_city", "manage_role")
    )
    main_win.manage_role.pushButton_12.clicked.connect(
        partial(travel, "new_depot", "manage_role")
    )

    ### new_city page buttons
    main_win.new_city.pushButton.clicked.connect(
        partial(travel, "profile", "new_city")
    )
    main_win.new_city.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "new_city")
    )
    main_win.new_city.pushButton_3.clicked.connect(
        partial(travel, "pickup", "new_city")
    )
    main_win.new_city.pushButton_4.clicked.connect(
        partial(travel, "receive", "new_city")
    )
    main_win.new_city.pushButton_7.clicked.connect(
        partial(doLogout, "new_city")
    )
    main_win.new_city.pushButton_8.clicked.connect(
        partial(travel, "manage_role", "new_city")
    )
    main_win.new_city.pushButton_9.clicked.connect(
        partial(travel, "assign_manager", "new_city")
    )
    main_win.new_city.pushButton_10.clicked.connect(expandCity)
    main_win.new_city.pushButton_12.clicked.connect(
        partial(travel, "new_depot", "new_city")
    )
    main_win.new_city.pushButton_13.clicked.connect(searchCity)

    ### new_depot page buttons
    main_win.new_depot.pushButton.clicked.connect(
        partial(travel, "profile", "new_depot")
    )
    main_win.new_depot.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "new_depot")
    )
    main_win.new_depot.pushButton_3.clicked.connect(
        partial(travel, "pickup", "new_depot")
    )
    main_win.new_depot.pushButton_4.clicked.connect(
        partial(travel, "receive", "new_depot")
    )
    main_win.new_depot.pushButton_7.clicked.connect(
        partial(doLogout, "new_depot")
    )
    main_win.new_depot.pushButton_8.clicked.connect(
        partial(travel, "manage_role", "new_depot")
    )
    main_win.new_depot.pushButton_9.clicked.connect(
        partial(travel, "assign_manager", "new_depot")
    )
    main_win.new_depot.pushButton_10.clicked.connect(inaugurateDepot)
    main_win.new_depot.pushButton_11.clicked.connect(
        partial(travel, "new_city", "new_depot")
    )
    ...
    main_win.assign_manager.pushButton.clicked.connect(
        partial(travel, "profile", "assign_manager")
    )
    main_win.assign_manager.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "assign_manager")
    )
    main_win.assign_manager.pushButton_3.clicked.connect(
        partial(travel, "pickup", "assign_manager")
    )
    main_win.assign_manager.pushButton_4.clicked.connect(
        partial(travel, "receive", "assign_manager")
    )
    main_win.assign_manager.pushButton_7.clicked.connect(
        partial(doLogout, "assign_manager")
    )
    main_win.assign_manager.pushButton_8.clicked.connect(
        partial(travel, "manage_role", "assign_manager")
    )
    main_win.assign_manager.pushButton_10.clicked.connect(getDepotDetails)
    main_win.assign_manager.pushButton_11.clicked.connect(
        partial(travel, "new_city", "assign_manager")
    )
    main_win.assign_manager.pushButton_12.clicked.connect(
        partial(travel, "new_depot", "assign_manager")
    )
    main_win.assign_manager.pushButton_13.clicked.connect(assignManager)

    ### receive page buttons
    main_win.receive.pushButton.clicked.connect(
        partial(travel, "profile", "receive")
    )
    main_win.receive.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "receive")
    )
    main_win.receive.pushButton_3.clicked.connect(
        partial(travel, "pickup", "receive")
    )
    main_win.receive.pushButton_5.clicked.connect(
        partial(travel, "manage_role", "receive")
    )
    ...
    main_win.receive.pushButton_9.clicked.connect(
        partial(travel, "dispatch", "receive")
    )
    main_win.receive.pushButton_10.clicked.connect(getDepotDetailsForRecieve)

    ### dispatch page buttons
    main_win.dispatch.pushButton.clicked.connect(
        partial(travel, "profile", "dispatch")
    )
    main_win.dispatch.pushButton_2.clicked.connect(
        partial(travel, "new_parcel", "dispatch")
    )
    main_win.dispatch.pushButton_3.clicked.connect(
        partial(travel, "pickup", "dispatch")
    )
    main_win.dispatch.pushButton_5.clicked.connect(
        partial(travel, "manage_role", "dispatch")
    )
    main_win.dispatch.pushButton_6.clicked.connect(dispatchParcel)
    main_win.dispatch.pushButton_7.clicked.connect(
        partial(doLogout, "dispatch")
    )
    main_win.dispatch.pushButton_8.clicked.connect(
        partial(travel, "receive", "dispatch")
    )
    main_win.dispatch.pushButton_10.clicked.connect(getDepotDetailsForDispatch)

    ### track_parcel page buttons
    main_win.track_parcel.pushButton.clicked.connect(
        partial(travel, "profile", "track_parcel")
    )
    main_win.track_parcel.pushButton_3.clicked.connect(
        partial(travel, "pickup", "track_parcel")
    )
    main_win.track_parcel.pushButton_4.clicked.connect(
        partial(travel, "receive", "track_parcel")
    )
    main_win.track_parcel.pushButton_5.clicked.connect(
        partial(travel, "manage_role", "track_parcel")
    )
    main_win.track_parcel.pushButton_7.clicked.connect(
        partial(doLogout, "track_parcel")
    )
    main_win.track_parcel.pushButton_8.clicked.connect(
        partial(travel, "new_parcel", "track_parcel")
    )
    main_win.track_parcel.pushButton_9.clicked.connect(
        partial(travel, "my_parcel", "track_parcel")
    )
